<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	//category 라는 파라미터명으로 전달된 문자열이 있는지 읽어와 본다.
	String category=request.getParameter("category");
	String board=request.getParameter("board");
	if(category==null){ //만일 전달된 내용이 없으면
		category="";  //빈 문자열을 대입한다. (NullPointerException 방지)
	}
%>
	...
	<div class="container">
		...
		<div class="collapse navbar-collapse" id="one">
			<ul class="nav navbar-nav">
				...
				<li <%if(category.equals("board")){%>class="active"<%}%>><a href="${pageContext.request.contextPath }/board/list.jsp">방명록</a></li>
			</ul>
		</div>
	</div>
</div>