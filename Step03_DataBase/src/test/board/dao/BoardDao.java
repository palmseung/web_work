...
public class BoardDao {
	...
	public BoardDto getData(int num) {
		...
		try {
			conn = new DbcpBean().getConn();
			String sql = "SELECT writer, title, content, pwd, TO_CHAR(regdate,'YYYY.MM.DD HH24:MI') As regdate"
					+ " FROM board_free"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
			// ? 에 값 바인딩 
	...
	public List<BoardDto> getList(){
		...
		try {
			conn = new DbcpBean().getConn();
			String sql = "SELECT num, writer, title, pwd, TO_CHAR(regdate,'YYYY.MM.DD HH24:MI') As regdate"
					+ " FROM board_free"
					+ " ORDER BY num DESC";
			pstmt = conn.prepareStatement(sql);
