...
@WebFilter(value = {"/shop/*", "/users/private/*", "/cafe/private/*"})
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest req, 
		 ...
		 *  원래 type인 HttpServletRequest type 으로 캐스팅한다.
		 */
		...
		//3.세션에 "id"라는 키값으로 저장된 문자열이 있는지 읽어오기
		String id=(String)session.getAttribute("id");
		if(id !=null) { //로그인이 된 경우, 필터는 할 일이 없지요
			//관여하지 않고 요청의 흐름 계속 진행시키기
			chain.doFilter(req, res);
		}else {//로그인이 안 된 경우, 로그인폼으로 보내버려야 함
			/*
			...
			//GET 방식 파라미터 문자열이 있으면 읽어오기(쿼리문자열은 num=1&name=coffee...이런 거...)
